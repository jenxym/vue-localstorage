...
export default {
  data() {
    return {
      valid: false,
      nameRules: [(v) => !!v || "A task name is required", (v) => /^[a-zA-Z0-9\u00C0-\u00FF ]+$/.test(v) || "Only alphanumerics characters"],
      infoRules: [(v) => !!v || "A task description is required", (v) => /^[a-zA-Z0-9\u00C0-\u00FF ]+$/.test(v) || "Only alphanumerics characters"],
      taskData: {
        taskName: "",
        taskInfo: "",
      },
